import { WEBSOCKET_CONNECT, WEBSOCKET_SEND, WEBSOCKET_CONNECT_SEND } from '../middleware/websocket';
...
function overTheSocketWithConnect(type, payload) {
  return {
    type: WEBSOCKET_CONNECT_SEND,
    payload: { type, payload }
  };
}
...
function singleDispatch(type, payload) {
  return dispatch => {
    dispatch(overTheSocketWithConnect(type, payload));
    dispatch({ type, payload });
  }
}
...
export function joinChatForRefresh(user, url) {
  return singleDispatch(USER_JOINED, { user, url });
}
